...
float timeStep = 70; //Animation time step = 50 m.sec
int look_radius = 200;
bool retarget = false;
...
bool loadModel(const char* fileName)
{
    ...
    animationScene = aiImportFile("Models/Dwarf/avatar_walk.bvh", aiProcessPreset_TargetRealtime_MaxQuality);
...
void loadGLTextures(const aiScene* scene)
{
	...
	for (unsigned int m = 0; m < scene->mNumMaterials; ++m)
	{
		...
		if (scene->mMaterials[m]->GetTexture(aiTextureType_DIFFUSE, 0, &path) == AI_SUCCESS)
		{
            ...
            char* dest = new char[sizeof(path.data) + 1];
            char* prefix = "./";
            strncpy(dest, prefix, sizeof(prefix));
            strcat(dest, path.data);
...
void updateNodeMatrices(int tick)
{
    ...
    aiAnimation* walkAnim = animationScene->mAnimations[0];
    ...
    for (int i = 0; i < anim->mNumChannels; i++)
    {
        ...
        if (ndAnim->mNumPositionKeys > 1) {
            for(int j = 1; j < ndAnim->mNumPositionKeys; j++) {
                if(ndAnim->mPositionKeys[j-1].mTime < tick && ndAnim->mPositionKeys[j].mTime >= tick) {
                    index = j;
                    break;
                }
            }
        ...
        if(i == 1 && retarget) index = 0;
        
        if (index < ndAnim->mNumPositionKeys) {
            aiVector3D posn = (ndAnim->mPositionKeys[index]).mValue;
            matPos.Translation(posn, matPos);
        }
        
        if (ndAnim->mNumRotationKeys > 1) {
            for(int j = 1; j < ndAnim->mNumRotationKeys; j++) {
                if(ndAnim->mRotationKeys[j-1].mTime < tick && ndAnim->mRotationKeys[j].mTime >= tick) {
                    index = j;
                    break;
                }
            }
        ...
        if(retarget) {
            if(NODE_NAMES.find(ndAnim->mNodeName.C_Str()) != NODE_NAMES.end()) {
                int channel = NODE_NAMES.at(ndAnim->mNodeName.C_Str());
                aiNodeAnim* walk = walkAnim->mChannels[channel];
                if (walk->mNumRotationKeys > 1) {
                    for(int j = 1; j < walk->mNumRotationKeys; j++) {
                        if(walk->mRotationKeys[j-1].mTime < tick%13 && walk->mRotationKeys[j].mTime >= tick%13) {
                            index = j;
                            break;
                        }
                        index = 0;
                ...
                rotn = (walk->mRotationKeys[index]).mValue;
            }
        }
    ...
    for (int n = 0; n < scene->mNumMeshes; n++)
    {
        ...
        for(int i = 0; i < mesh->mNumBones; i++) 
        {
            ...
            for(int k = 0; k < bone->mNumWeights; k++)
            {
                ...
                mesh->mVertices[vid] = prodMatrix * vert;
...
void display()
{
    ...
    gluLookAt((posn.x - xc) * tmp, 0, (posn.z - zc + look_radius) * tmp, (posn.x - xc) * tmp, 0, (posn.z - zc) * tmp, 0, 1, 0);
